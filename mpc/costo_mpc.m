...
%   alpha = 5000: Dominante. Maximiza progreso de secado (prioridad #1).
%   delta = 500:  Tracking cuadratico asimetrico hacia X_ref.
%   gamma = 0.5:  Bajo. Penaliza energia sin impedir el secado.
...
% =========================================================================
    ...
    % v5.4 FIX: Cambio a Norma L1 (abs) en vez de L2 (cuadrada).
    % Esto permite saltos pronunciados de una vez (ej. 0 a 100) pero 
    % penaliza severamente la oscilacion (0-100-0-100).
    if isfield(estado, 'pwm_anterior')
        pwm_completo = [estado.pwm_anterior; pwm_extendido];
    else
        pwm_completo = [0; pwm_extendido];
    end
    delta_pwm = diff(pwm_completo);
    J_suavidad = params.beta * sum(abs(delta_pwm));
