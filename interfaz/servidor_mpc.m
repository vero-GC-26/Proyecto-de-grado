...
function res = ejecutar_batch(cfg, base_dir, status_file)
    ...
    % ═══════════════════════════════════════════════════════════════════
    % CORTE MAESTRO: Desactivado por auditoría (sin corte en la meta)
    % ═══════════════════════════════════════════════════════════════════
    idx_corte = []; %find(res_mpc_20.X_promedio <= mpc_params.X_ref, 1);
    if ~isempty(idx_corte) && idx_corte < N_horas
        fprintf('  [CORTE] X_ref=%.3f alcanzada en hora %d (Dia %.1f). Apagando ventilador.\n', ...
            mpc_params.X_ref, idx_corte, idx_corte/24);
        
        % 1. Cortar vectores de velocidad y comando PWM
        v_mpc_vec(idx_corte:end) = 0;
        pwm_hist(idx_corte:end)  = 0;
        
        % 2. Cortar vfd_hist y frac_hist para que el cálculo de energía sea correcto
        if isfield(info_stats, 'vfd_hist') && length(info_stats.vfd_hist) >= idx_corte
            info_stats.vfd_hist(idx_corte:end) = 0;
            info_stats.frac_hist(idx_corte:end) = 0;
        end
        
        % 3. Corregir clima_replay: deshacer efecto HX en horas post-corte
        if isfield(cfg, 'use_hx') && cfg.use_hx == 1 && isfield(clima, 'T_amb')
            for ih = idx_corte:min(N_horas, clima.n_horas)
                clima_replay.T_amb(ih) = clima.T_amb(ih);
                clima_replay.HR(ih)    = clima.HR(ih);
            end
        end
        
        % 4. Re-simular planta 20-capas con perfil de velocidad corregido
        res_mpc_20 = gemelo_camara_forzada(params_cin, datos, camara, clima_replay, v_mpc_vec);
        
        % 5. Actualizar todos los campos del resultado con datos corregidos
        res.mpc.X_hist        = res_mpc_20.X_promedio(:)';
        res.mpc.T_hist        = res_mpc_20.T_promedio(:)';
        res.mpc.X_final       = res_mpc_20.X_promedio(end);
        res.mpc.X_capas_final = res_mpc_20.X(end, :);
        res.mpc.pwm_hist      = pwm_hist;
        res.mpc.pwm_promedio  = mean(pwm_hist);
        
        % 6. Recalcular energía con las señales corregidas (post-corte)
        if isfield(info_stats, 'vfd_hist')
            res.mpc.vfd_hist    = info_stats.vfd_hist(:)';
            res.mpc.frac_hist   = info_stats.frac_hist(:)';
            res.mpc.energia_kWh = sum(cfg.P_fan * (res.mpc.vfd_hist/100).^3 .* res.mpc.frac_hist) / 1000;
        ...
        % 7. Recalcular ahorro energético
        if res.constante.energia_kWh > 0
            res.mpc.ahorro_pct = (1 - res.mpc.energia_kWh / res.constante.energia_kWh) * 100;
        end
        
        % 8. Actualizar T_hist_hx si el intercambiador estaba activo
        if isfield(cfg, 'use_hx') && cfg.use_hx == 1 && isfield(clima, 'T_amb')
            res.mpc.T_hist_hx = clima_replay.T_amb(1:N_horas)';
        end
        
        fprintf('  [CORTE OK] pwm_prom=%.1f%%, E=%.3f kWh, Ahorro=%.1f%%\n', ...
            res.mpc.pwm_promedio, res.mpc.energia_kWh, res.mpc.ahorro_pct);
    else
        if isempty(idx_corte)
            fprintf('  [INFO] X_ref=%.3f no alcanzada en %d horas (X_final=%.4f). Sin corte.\n', ...
                mpc_params.X_ref, N_horas, res_mpc_20.X_promedio(end));
        end
    end
    % ═══════════════════════════════════════════════════════════════════
